...
describe('QuickBooks documented full updates', () => {
  ...
  it('reads, verifies, preserves, merges, and posts a complete entity', async () => {
    ...
    const result = await executeQuickBooksFullUpdate({
      params: { accessToken: 'token', realmId: '123', privateNote: 'new note' },
      entity: 'Bill',
      resource: 'bill',
      recordId: 'bill-1',
      syncToken: '2',
      buildPatch: (params) => ({ sparse: true, PrivateNote: params.privateNote }),
    })
  ...
  it('rejects a stale sync token without posting an update', async () => {
    ...
    await expect(
      executeQuickBooksFullUpdate({
        params: { accessToken: 'token', realmId: '123' },
        entity: 'Bill',
        resource: 'bill',
        recordId: 'bill-1',
        syncToken: '2',
